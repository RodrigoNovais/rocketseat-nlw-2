import { Context, Next } from 'koa'

import connection from '../database/connection'
import convertHourToMinutes from '../utils/convertHourToMinutes'

export type ScheduleItem = {
    week_day: number
    from: string
    to: string
}

export const index = async (context: Context, next: Next) => {
    const { week_day, subject, time } = context.request.query

    if (!week_day || !subject || !time) {
        context.status = 400
        context.body = { error: 'Missing filters to search classes' }

        return next()
    }

    const timeInMinutes = convertHourToMinutes(time)

    const classes = await connection('classes')
        .whereExists(function() {
            this
                .select('class_schedule.*')
                .from('class_schedule')
                .whereRaw('`class_schedule`.`class_id` = `classes`.`id`')
                .whereRaw('`class_schedule`.`week_day` = ??', [Number(week_day)])
                .whereRaw('`class_schedule`.`from` <= ??', [timeInMinutes])
                .whereRaw('`class_schedule`.`to` > ??', [timeInMinutes])
        })
        .where('classes.subject', '=', subject)
        .join('users', 'classes.user_id', '=', 'users.id')
        .select(['classes.*', 'users.*'])

    context.body = classes

    return next()
}

export const store = async (context: Context, next: Next) => {
    const { name, avatar, whatsApp, bio, subject, cost, schedule } = context.request.body

    const trx = await connection.transaction()

    try {
        const [user_id] = await trx('users')
            .insert({ name, avatar, whatsApp, bio })

        const [class_id] = await trx('classes')
            .insert({ subject, cost, user_id })

        const classSchedule = schedule.map((scheduleItem: ScheduleItem) => {
            return {
                class_id,
                week_day: scheduleItem.week_day,
                from: convertHourToMinutes(scheduleItem.from),
                to: convertHourToMinutes(scheduleItem.to),
            }
        })

        await trx('class_schedule').insert(classSchedule)
        await trx.commit()

        context.status = 201

        return next()
    } catch (err) {
        await trx.rollback();

        context.status = 400
        context.body = { error: 'Unexpected error while creating new class' }

        return next()
    }
}

export default { index, store }
